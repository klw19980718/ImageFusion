...
export default function PricingSection() {
  ...
  const handleUpgradeClick = async (priceId: string, planKey: string) => {
    ...
    if (!userId) {
      ...
      // 可以选择提示用户或记录错误
      alert(t('error.missingUserId', { defaultMessage: 'Could not get user information. Please try refreshing the page.' }));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: t('error.generic') })); 
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {pricingPlans.map((plan) => {
              ...
              if (!planDetails || typeof planDetails !== 'object' || !('title' in planDetails) || !('price' in planDetails) || !('features' in planDetails) || !Array.isArray(planDetails.features)) {
                console.warn(`Pricing plan details missing or malformed for key: ${plan.key}`);
                return null;
              }
