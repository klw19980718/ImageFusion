...
export default function ComparisonSlider() {
  ...
  return (
    <section id="examples" className="py-20 px-6 bg-slate-100">
...
function SingleComparison({ image }: { image: ExampleImage }) {
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full aspect-[2/3] overflow-hidden rounded-2xl shadow-custom select-none"
    >
      {/* 原始图片 (底层) */}
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center pointer-events-none">
      ...
      {/* 标签 (调整位置) */}
      <div className="absolute top-4 left-4 bg-primary/70 text-white px-3 py-1 rounded-full text-sm font-medium pointer-events-none z-20">
        {t('generatedLabel')} 
      </div>
